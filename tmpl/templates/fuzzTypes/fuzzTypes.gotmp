...
type (
	...
	Match struct {
		...
		Time  struct {
			DownBound time.Duration `json:"down_bound,omitempty"`
			UpBound   time.Duration `json:"up_bound,omitempty"`
		} `json:"time,omitempty"`
	}
	...
	Fuzz struct {
		...
		Preprocess struct {
			...
			Preprocessors string                 `json:"preprocessors,omitempty"` // 使用的自定义预处理器
