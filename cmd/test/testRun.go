...
var subCmdRun = &cobra.Command{
	...
	passed to fgpk by -p flag and fgpk will use it). in this way, some types of plugin
	has struct argument will use empty structs. also you can't specify expect values
	for each test. you can still specify multiple tests by one -e, however. you can do
	so by using comma-separated pseudo function expressions, for example, you can
	specify -e "test(1,2,3),test(4,5,6),test(7,8,9),..." to run tests with different
	arguments.
